...
def test_caches_shared_vds_reads_once(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "ctm_for_dwc.utils.ctm.validation.pd.read_csv",
        counting,
    )
...
CTMSIM_CONFIGS = REPO / "src/ctm_for_dwc/utils/ctm/ctmsim_configs"
CTMSIM_RESULTS = REPO / "src/ctm_for_dwc/utils/ctm/ctmsim_results"
...
def test_qq_density_samples_use_window_mean(tmp_path):
    from ctm_for_dwc.utils.ctm import compute_qq_samples
