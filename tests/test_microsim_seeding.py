...
def test_seed_count_matches_expected_for_constant_inflow():
    ...
    veh = seed_vehicles(rate, spec, v_f_ms=60 * MPH_TO_MS, eta_ev=0.5)
...
def test_seed_v_max_within_band():
    ...
    veh = seed_vehicles(rate, spec, v_f_ms=v_f, eta_ev=0.3)
...
def test_seed_ev_fraction_matches_eta():
    ...
    veh = seed_vehicles(rate, spec, v_f_ms=60 * MPH_TO_MS, eta_ev=0.25)
    assert veh.is_ev.mean() == pytest.approx(0.25, abs=0.05)


def test_seed_is_reproducible_under_seed():
    rate = np.full(1000, 700.0)
    a = seed_vehicles(rate, _spec(seed=42), v_f_ms=60 * MPH_TO_MS, eta_ev=0.4)
    b = seed_vehicles(rate, _spec(seed=42), v_f_ms=60 * MPH_TO_MS, eta_ev=0.4)
...
def test_seed_different_seeds_differ():
    rate = np.full(1000, 700.0)
    a = seed_vehicles(rate, _spec(seed=1), v_f_ms=60 * MPH_TO_MS, eta_ev=0.4)
    b = seed_vehicles(rate, _spec(seed=2), v_f_ms=60 * MPH_TO_MS, eta_ev=0.4)
