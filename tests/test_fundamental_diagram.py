...
1. **Primitive tests** (this layer) build small synthetic DataFrames in
   ...
   :func:`estimate_free_flow_speed`,
   :func:`estimate_congestion_wave_speed`,
   :func:`find_fundamental_diagram_peak`, and
   :func:`fit_fundamental_diagram` recover the
...
2. **Batch / plot tests** (in :mod:`tests.test_calibrate_fundamental_diagrams`)
   exercise the full :func:`calibrate_fundamental_diagrams` pipeline against
   ``tmp_path`` CSVs.
...
def _congestion_points(
    ...
    :func:`estimate_congestion_wave_speed`'s IQR-fence step has the column it
...
def _triangular_fd_points(
    ...
    :func:`estimate_congestion_wave_speed` has its input.
    """
    rng = np.random.default_rng(seed)
    half = n // 2
    ...
    # `fit_fundamental_diagram` seeds correctly.
...
def test_estimate_free_flow_speed_recovers_ground_truth_slope():
    df = _free_flow_points(n=50)
    v_f_hat, model = estimate_free_flow_speed(
        df=df,
...
def test_estimate_free_flow_speed_drops_congestion_points():
    ...
    v_f_hat, _ = estimate_free_flow_speed(
        df=df,
        critical_density_estimate=RHO_CRIT,
    )
    assert v_f_hat == pytest.approx(V_F, rel=1e-6)
...
def test_estimate_congestion_wave_speed_recovers_wave_and_jam():
    ...
    slope, jam, model, bin_df = estimate_congestion_wave_speed(
        df=df,
        critical_density_estimate=RHO_CRIT,
        bin_size=1,
    )
...
def test_estimate_congestion_wave_speed_drops_free_flow_points():
    ...
    slope, jam, _model, _bins = estimate_congestion_wave_speed(
        df=df,
        critical_density_estimate=RHO_CRIT,
        bin_size=1,
    )
...
def test_estimate_congestion_wave_speed_bin_size_changes_bin_count():
    """100 points / bin_size = expected number of bins."""
    df = _congestion_points(n=100)
    _slope, _jam, _model, bins_5 = estimate_congestion_wave_speed(
        df=df, critical_density_estimate=RHO_CRIT, bin_size=5,
    )
    _slope, _jam, _model, bins_20 = estimate_congestion_wave_speed(
        df=df, critical_density_estimate=RHO_CRIT, bin_size=20,
    )
...
def test_find_peak_intersects_free_flow_and_congestion_lines():
    """Composite test: pipe both fits into the peak finder, recover (rho_crit, q_max)."""
    _v_f, ff_model = estimate_free_flow_speed(
        df=_free_flow_points(n=50),
        critical_density_estimate=RHO_CRIT,
    )
    _slope, _jam, cong_model, _ = estimate_congestion_wave_speed(
        df=_congestion_points(n=100),
        critical_density_estimate=RHO_CRIT,
        bin_size=1,
    )
    rho_crit_hat, q_max_hat = find_fundamental_diagram_peak(
...
def test_find_peak_handles_arbitrary_slopes_and_intercept():
    ...
    rho, q = find_fundamental_diagram_peak(ff, cong)
...
def test_fit_recovers_all_five_fd_parameters():
    """End-to-end recovery on a high-resolution triangular FD.

    The default ``bin_size=10`` inside :func:`estimate_congestion_wave_speed`
