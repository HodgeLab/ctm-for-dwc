...
class Vehicles:
    ...
    def __post_init__(self) -> None:
        ...
        if self.v_max.shape[0] != n or self.is_ev.shape[0] != n:
            raise ValueError(
                "Vehicles.entry_step, v_max, is_ev must have the same length; "
                f"got {self.entry_step.shape[0]}, {self.v_max.shape[0]}, "
                f"{self.is_ev.shape[0]}"
            )
