...
def corpus_digest(data: TrainingData) -> str:
    """sha256 over the row-aligned corpus arrays (NaNs hash stably)."""
    h = hashlib.sha256()
    for arr in (data.X, data.alpha, data.r_true, data.s_true, data.q_up,
                data.q_down, data.c_w, data.stretch_id,
                data.feasible, data.alpha_clipped):
        h.update(np.ascontiguousarray(arr).tobytes())
    return h.hexdigest()


def write_manifest(path, *, corpus_params: dict, split_seed: int,
                   val_stretch: int, test_stretch: int, data: TrainingData) -> dict:
    ...
    ``window_size``, ``require_both_measured``, ``keep_infeasible``,
...
def rebuild_corpus(manifest: dict, *, stretches=None, timeseries_dir=None,
                   station_meta=None) -> TrainingData:
    """Re-assemble the corpus from the manifest's recorded parameters and verify
    the digest. Keyword overrides substitute machine-specific paths only."""
    c = manifest["corpus"]
    ...
    data = build_training_data(
        ...
        keep_infeasible=c["keep_infeasible"],
        record_start=c["record_start"], record_end=c["record_end"],
    )
    if len(data.alpha) != manifest["n_rows"] or corpus_digest(data) != manifest["digest"]:
        raise ValueError(
