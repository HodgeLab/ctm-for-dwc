...
* :func:`plot_measured_heatmap`      -- direct/tiebreak cell x 5-min
...
def plot_measured_heatmap(
    grid: np.ndarray, *,
    row_labels: list[str], times: pd.DatetimeIndex,
    title: str, cbar_label: str, out_path: Path,
    cmap: str = "viridis",
) -> None:
    """Space-time heatmap of a measured PeMS quantity (rows x 5-min columns).

    ``grid`` is ``(n_rows, n_cols)`` with one row per direct/tiebreak VDS
    cell and one column per 5-min window; NaN cells (invalid observed
    sample) render blank. ``row_labels`` annotate each row; ``times``
    labels the x-axis at each window start.
    """
    n_rows, n_cols = grid.shape
    fig, ax = plt.subplots(
        figsize=(min(14, 2 + 0.5 * n_cols), 1.5 + 0.4 * n_rows)
    )
    ...
    ax.set_xticklabels(
        [t.strftime("%H:%M") for t in times], rotation=90, fontsize=8,
    )
    ax.set_xlabel("5-min window start")
    ax.set_title(title)
    fig.colorbar(mesh, ax=ax, label=cbar_label)
    fig.tight_layout()
    fig.savefig(out_path, dpi=120)
